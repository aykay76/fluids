...
void CfluidsView::OnDraw(CDC* pDC)
{
	...
	if (bmp)
	{
		...
		{
			unsigned char d = max(0, min(255, (unsigned char)(dens[i] * 1000.0)));
			cp[i] = (d << 8) | (0xff << 24);
		}
	...
	if (bmp)
	{
		...
		for (int i = 0; i < size; i++)
		{
			unsigned char r = max(0, min(255, (unsigned char)(u[i] * 1000.0)));
			unsigned char b = max(0, min(255, (unsigned char)(v[i] * 1000.0)));
	...
	CString str;
	str.Format(L"%.6f -> %d", dens[IX(3, 3)], (int)(min(255, dens[IX(3, 3)] * 1000.0)));
...
//	dens_prev[IX(10, 10)] = 100000.0;
	...
	{
		if (m_leftButtonDown) dens_prev[IX(m_pt.x, m_pt.y)] = 100.0;
		u_prev[IX(m_pt.x, m_pt.y)] = (m_pt.x - ptLast.x) * 50.0;
		v_prev[IX(m_pt.x, m_pt.y)] = (m_pt.y - ptLast.y) * 50.0;
